...
void check_values (){
    ...
    while(rules_vec[i].sensor != NULL){
        ...
        if (strstr(rules_vec[i].sensor->name, "TEMP") != NULL)
        {
            strcpy(grandeza,"TEMP");
            //printf("T   ");
        }else if (strstr(rules_vec[i].sensor->name, "LIGHT") != NULL)
        {
            strcpy(grandeza,"LIGHT");
            //printf("L   ");
        }else if (strstr(rules_vec[i].sensor->name, "HUM") != NULL)
        {
            strcpy(grandeza,"HUM");
            //printf("H   ");
        } else if (strstr(rules_vec[i].sensor->name, "CURR") != NULL)
        {
            strcpy(grandeza,"CURR");
            //printf("C   ");
        } else if (strstr(rules_vec[i].sensor->name, "VOLT") != NULL)
        {
            strcpy(grandeza,"VOLT");
            //printf("V   ");
        }else
        {
            printf("Error check_values 1\n");
        }*/
...
void new_values(int moteID)
{
    ...
    while (i < n_sensores){
        ...
        if (strcmp(sensores[i],"1")==0){
            ...
            int ii=0;
            while (strncmp(motes[moteID].pos[ii].name,"\0",3)){
    ...
    for (int i=0; i<N_ACTUATORS; i++){
        ...
        if (strstr(outputs_vetor[i].name, str)!=NULL && strstr(outputs_vetor[i+1].name, str)==NULL && strncmp(outputs_vetor[i].name,"\0",3)){
            ...
            for (int j=N_ACTUATORS; j>i+1; j--){
                //printf("i=%i; j=%i\n", i,j);
                if (!strncmp(outputs_vetor[j-1].name,"\0",3)) continue;
    ...
    if (par==0){
        ...
        for (int i=0; i<celulas_matriz; i++){
            ...
            for(int k=0; k<2; k++){
                ...
                if(strcmp(outputs_vetor[i].name, "Skipped")==0) fprintf(fp, "%s,", BLACK);
...
void outputs_update(void){
    ...
    for (int i=0; i<MAX_RULES; i++){
        ...
        if (rules_vec[i].operation==0){
            ...
            if (!strcmp(&rules_vec[i].operation, ">") && rules_vec[i].sensor->value > rules_vec[i].ref && !strcmp(&rules_vec[i].operation2, ">") && rules_vec[i].sensor2->value > rules_vec[i].ref2 && !strcmp(rules_vec[i].op, "AND")){
                *rules_vec[i].out=1;
            }
            if (!strcmp(&rules_vec[i].operation, ">") && rules_vec[i].sensor->value > rules_vec[i].ref && !strcmp(&rules_vec[i].operation2, "<") && rules_vec[i].sensor2->value < rules_vec[i].ref2 && !strcmp(rules_vec[i].op, "AND")){
                *rules_vec[i].out=1;
            ...
            if ((!strcmp(&rules_vec[i].operation, ">") && rules_vec[i].sensor->value > rules_vec[i].ref) || (!strcmp(&rules_vec[i].operation2, ">") && rules_vec[i].sensor2->value > rules_vec[i].ref2 && !strcmp(rules_vec[i].op, "OR"))){
                *rules_vec[i].out=1;
            }
            if ((!strcmp(&rules_vec[i].operation, ">") && rules_vec[i].sensor->value > rules_vec[i].ref) || (!strcmp(&rules_vec[i].operation2, "<") && rules_vec[i].sensor2->value < rules_vec[i].ref2 && !strcmp(rules_vec[i].op, "OR"))){
                *rules_vec[i].out=1;
